...
Do not use this skill for Bugbot or security review (those have their own subagents). Do not use it as the top-level workflow for Linear plan/implement/submit (`linear-issue` calls this skill as a pre-merge gate). Still use this skill when the user asks to review a PR or diff, including one tied to `SRES-#`. Do not use it to author `AGENTS.md` (that is `write-agents-docs`). Do not use it to retro a conversation for `AGENTS.md` or skill gaps (that is `agent-retro`). Do not use it for repo-wide crate, CI, or skill housekeeping (that is `health-audit`).
