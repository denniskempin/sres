...
description: "Write or refresh AGENTS.md files and //! file headers in the SRES repo. Use after an architectural or test-strategy change, when a directory lacks an AGENTS.md, or when applying agreed AGENTS.md edits. Do not use to mine a conversation for what to document (that is agent-retro), to look up Linear usage (linear), or for periodic crate/CI/skill housekeeping (that is health-audit)."
---
