...
5. `krom_hdma_redspace` is the HDMA golden (CGRAM gradient via channel 0). A black image means HDMA did not run. Pair `run_framebuffer_test(..., N)` with snapshot capture of frame `N-1` (`sres_emulator/src/AGENTS.md` Behaviors).
