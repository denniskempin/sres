...
| `{name}.writes` | Bitcode `$2100–$213F` writes (`master_clock`, offset, value) for that frame; missing file is empty |
...
5. `krom_hdma_redspace` is the HDMA golden (CGRAM gradient via channel 0). A black image means HDMA did not run. `test_krom_hdma_redspace_snapshot_replay` captures `v=0` + writes from the same ROM and replays on a bare `Ppu` against `krom_hdma_redspace-framebuffer.png`.
