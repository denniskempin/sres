...
| `snes_header_irq.asm` | Same header; native IRQ is label `IrqHandler` (including ROM must define it) |
...
3. `base.asm` always includes `snes_header.asm`, not the vector variants. `rom_tests/krom_ret.asm` includes `snes_header_ret.asm` and defines `RTIBreak`. `rom_tests/wai_nmi.asm` includes `snes_header_nmi.asm` and defines `NmiHandler`. `rom_tests/wai_irq.asm` includes `snes_header_irq.asm` and defines `IrqHandler`.
