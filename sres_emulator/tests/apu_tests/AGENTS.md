...
| Prefix | Owns |
|--------|------|
| `play_brr_sample` | `.sfc`/`.spc`/`.brr`/`.asm` and golden `.wav` |
| `play_noise` | `.sfc`/`.spc`/`.asm`; no `.wav` |
...
3. `play_brr_sample*.asm` includes `../asm_lib/`. `play_noise*.asm` includes `lib/` (no `apu_tests/lib`; assemble with include root `sres_emulator/tests`).
