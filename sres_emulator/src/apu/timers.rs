    ...
    pub fn update(&mut self, spc_cycles: u32) {
        ...
        while self.base_counter >= self.base_divisor {
            ...
            if self.enabled {
                ...
                let target = if self.interval_target == 0 { 256 } else { self.interval_target as u16 };
...
pub struct ApuTimers {
    timers: [ApuTimer; 3],
    control_register: u8, // $F1 register
    _debug_event_collector: DebugEventCollectorRef<ApuBusEvent>,
}
...
impl ApuTimers {
    pub fn new(debug_event_collector: DebugEventCollectorRef<ApuBusEvent>) -> Self {
        Self {
            ...
            control_register: 0,
            _debug_event_collector: debug_event_collector,
        }
    }
    ...
    pub fn write_control(&mut self, value: u8) {
        ...
        // Check for enable transitions (0->1)
        for (i, timer) in self.timers.iter_mut().enumerate() {
            let new_enabled = (value >> i) & 1 != 0;
...
mod tests {
    ...
    fn test_timer_basic_functionality() {
        ...
        let mut timers = ApuTimers::new(DebugEventCollectorRef(debugger));
        
        // Test initial state
        assert_eq!(timers.read_control(), 0);
    ...
    fn test_timer_enable_disable() {
        ...
        // Enable timer 0
        timers.write_control(0x01);
        assert_eq!(timers.read_control(), 0x01);
        
        // Disable timer 0
        timers.write_control(0x00);
        assert_eq!(timers.read_control(), 0x00);
    }
    ...
    fn test_timer_target_setting() {
        ...
        // Enable timers
        timers.write_control(0x07); // Enable all 3 timers
