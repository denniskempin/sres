...
6. Frame swap latches on `Clock` vblank rise (`consume_vblank`) so a `Waiting` idle that spans vblank still captures the frame. A pending video frame not consumed by `swap_video_frame` is overwritten on the next vblank rise. `execute_frames(N)` stops when `f >= start+N` (typically `v≈0` of that frame), so `swap_video_frame` then yields visible frame `N-1`.
...
| `main_bus/` | Memory map, DMA, HDMA, device wrappers, interrupts. |
