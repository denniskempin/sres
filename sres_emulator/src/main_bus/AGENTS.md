...
- NMITIMEN bit 0 writes emit `JoypadAutoReadEnable`. `$4200`/`$4207`–`$420A` reads use the register catch-all (not `Clock`, which would panic). `$4210`–`$4212` writes use the register catch-all.
