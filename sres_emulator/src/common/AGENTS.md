...
- Components emit through `DebugEventCollectorRef` (`on_event` / `on_event_with` / `on_error` / `on_unimplemented`). `Debugger` stores events in `RingBuffer` and unimplemented hits in a `HashMap`.
