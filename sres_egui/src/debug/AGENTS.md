...
| `event.rs` | `event_filter_widget` (`EventFilter` parse + quick-add) |
...
3. Step / Step Frame / Step Scanline in `debug_controls_widget` are enabled only while paused and return `DebugCommand` (`cpu.rs:90-102`).
4. `event_filter_widget` mutates the `Vec<EventFilter>` the caller passes: parent right panel uses `break_points` (`debug.rs:166`); `LogViewer` uses `log_points` (`log_viewer.rs:31`). Quick-add "Step" pushes `CpuProgramCounter(0..u32::MAX)` / `Spc700ProgramCounter(0..u16::MAX)`, not `EventFilter::CpuStep` / `Spc700Step` (`event.rs:79`, `event.rs:105`).
5. `LogViewer` iterates `log.stack` reversed (oldest first) with `stick_to_bottom` (`log_viewer.rs:42-55`).
6. `PpuSpritesWidget` updates textures only for visible table rows (`ppu.rs:214-224`). `PpuDebugWindow` textures start as `ColorImage::example` until first `show` (`ppu.rs:97`).
...
- `cpu_widgets`, `ppu_widgets`, `apu_widgets`, `log_line_events`.
