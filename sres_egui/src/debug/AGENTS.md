...
| `unimplemented.rs` | `UnimplementedViewer` hit counts; checkbox toggles `EventFilter::Unimplemented` on `break_points` |
...
4. `event_filter_widget` mutates the `Vec<EventFilter>` the caller passes and reads caller-owned `EventFilterInputState`: parent right panel uses `break_points` (`debug.rs:173-177`); `LogViewer` uses `log_points` (`log_viewer.rs:40`). Quick-add "Step" pushes `CpuProgramCounter(0..u32::MAX)` / `Spc700ProgramCounter(0..u16::MAX)`, not `EventFilter::CpuStep` / `Spc700Step` (`event.rs:88`, `event.rs:115`).
5. `LogViewer` iterates `log.stack` reversed (oldest first) with `stick_to_bottom` (`log_viewer.rs:51-64`). `UnimplementedViewer` lists `debugger.unimplemented_hits()` only (not the ring). Its "Break when reached" checkbox adds/removes `EventFilter::Unimplemented` on `break_points` (`unimplemented.rs:31-41`), same list as the right-panel breakpoint widget.
